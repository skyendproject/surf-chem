...
export async function getFormulationTypes() {
    const docs = await readCollection('FormulationTypes', [orderBy('sort_order')]);
...
export async function getProductSeries() {
    const docs = await readCollection('ProductSeries', [orderBy('sort_order')]);
    const activeDocs = docs.filter(d => d.active);
    return activeDocs.map(d => {
...
export async function getProductClasses() {
    const docs = await readCollection('ProductClasses', [orderBy('title')]);
    const activeDocs = docs.filter(d => d.active);
    return activeDocs.map(d => {
        return {
            id: d.id,
...
export async function getProduct(id) {
    ...
    return {
        ...
        manufactured_at: d.manufactured_at.toDate().toLocaleString(),
        name: d.name,
...
export async function getProducts() {
    const docs = await readCollection('Products', [orderBy('sort_order')]);
    const activeDocs = docs.filter(d => d.active);
    const shortDocs = activeDocs.map(d => {
        return {
            id: d.id,
            name: d.name,
...
export async function getFormulations() {
    const docs = await readCollection('FormulationGuides', [orderBy('sort_order')]);
    const activeDocs = docs.filter(d => d.active);
    return activeDocs.map(d => {
        return {
            id: d.id,
            ...
            code: d.code || '',
            type: _formulationsTypesCache[d?.formulation_type?.id] || d?.formulation_type?.id || '',
            ...
            parameters: (d.parameters || []).map(f => ({
                title: f.title || '',
                type: f.function || '',
                result: f.result || '',
            })),
            issue_no: String(d.issue_no || 0),
            issued_at: d.issued_at.toDate().toLocaleString(),
        }
    })
}
...
export async function getEventBar() {
    const d = await readDocument('default', 'EventBar')
    return {
        title: d.title,
